...
def prototype_logits(label_embeddings: torch.Tensor, prototypes: torch.Tensor, sim: str) -> torch.Tensor:
    # label_embeddings: [B, L, D], prototypes: [2, D]
    if sim == "cosine":
...
def infer_one_release(
    ...
    with open(output_csv, "w", encoding="utf-8", newline="") as fout:
        ...
        with torch.no_grad():
            prototypes = model.label_embedding.weight  # [2, D]
            ...
            for batch in loader:
                ...
                logits = prototype_logits(pred_x0, prototypes, sim=sim)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prototype-similarity one-step inference for CodeDiff checkpoint")
    ...
    parser.add_argument(
        "--output-subdir",
        default="one-step-prototype",
        help="Subdir under prediction experiment folder (default: one-step-prototype)",
    )
    parser.add_argument(
        "--sim",
        choices=["dot", "cosine"],
...
def main() -> None:
    ...
    for release in test_releases:
        ...
        print(f"saved prototype one-step predictions -> {out_csv}")
