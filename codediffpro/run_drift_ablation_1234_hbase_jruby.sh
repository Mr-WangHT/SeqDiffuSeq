...
OUT_ROOT="codediffpro/output/loss/drift_ablation_1234_${RUN_TAG}"
mkdir -p "${OUT_ROOT}"
...
# Group 1: baseline
# Group 2: weaker hard negatives
# Group 3: group2 + stronger consistency
# Group 4: group3 + stronger regularization
set_group_params() {
  local group_id="$1"
  case "${group_id}" in
    1)
...
summary_path = out / "drift_ablation_1234_summary.csv"
if rows:
